# ViaScientific/cellSeq_Pipeline: Local Configuration
...
### Automatic
This pipeline itself needs no installation - NextFlow will automatically fetch it from GitHub if `ViaScientific/cellSeq_Pipeline` is specified as the pipeline name.
...
```
nextflow main.nf -profile docker --DOWNDIR /path/to/save/genome_data --reads '*_R{1,2}.fastq.gz' --genome_build c_elegans_ws279_refseq_PRJNA13758
```
...
```
nextflow main.nf -profile singularity --DOWNDIR /path/to/save/genome_data --reads '*_R{1,2}.fastq.gz' --genome_build c_elegans_ws279_refseq_PRJNA13758
```